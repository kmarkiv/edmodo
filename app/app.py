...
app.config['SQLAlchemy_DATABASE_URI'] = "mysql://%s:%s@127.0.0.1/%s" % (
    app.config['DATA']['username'], app.config['DATA']['password'], app.config['DATA']['database'])
...
def teachers():
    essay = db.execute(
        "SELECT *,CASE WHEN CURDATE() > due_date THEN 1 ELSE 0 END as past_due FROM homeworks WHERE teacher_id=%s" % current_user.data.id)
...
def teachers_homework(homework_id):
    sql = "SELECT *,CASE WHEN CURDATE() > due_date THEN 1 ELSE 0 END as past_due FROM homeworks WHERE id='%s'" % (homework_id)
    homework = db.execute(sql).first()
    sql = "SELECT * , COUNT( submission ) AS submits FROM (SELECT hs.id,s.homework_id,h.due_date,h. title,h.question,hs.updated_at as updated_at,u.name, u.email, s.student_id, hs.submission FROM users u, homeworks h, homework_students s LEFT JOIN homework_submissions hs ON s.student_id = hs.student_id AND hs.homework_id=%s WHERE s.homework_id = h.id AND h.id =%s AND u.id = s.student_id ORDER BY hs.id DESC) tmp GROUP BY student_id" % (homework_id,homework_id)
...
def teachers_homework_student(homework_id, student_id):
    ...
    sql = "SELECT h.*,u.name,u.email FROM homework_submissions h,users u WHERE h.homework_id=%s AND u.id=h.student_id AND student_id=%s ORDER BY h.id DESC" % (
...
@login_required
def students():
    sql = "SELECT s.updated_at as updated,CASE WHEN CURDATE() > h.due_date THEN 1 ELSE 0 END as past_due,h.*,COUNT(submission) as submits FROM homework_students s,homeworks h LEFT JOIN homework_submissions hs ON hs.homework_id=h.id WHERE s.homework_id=h.id AND h.id AND s.student_id=%s GROUP BY s.homework_id" % (
